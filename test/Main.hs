...
main = hspec $ do
  ...
  describe "Circular newtype" $ do
    ...
    describe "Compatible with inner Enum instance" $ do
      it "Successors" $
        iterate succ (Circular minBound)
          `startShouldBe` fmap Circular (cycle allDirs)
      it "Predecessors" $
        iterate pred (Circular maxBound)
          `startShouldBe` fmap Circular (cycle $ reverse allDirs)
